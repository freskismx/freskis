const APP = {
  ...
  resolveMenuImage(item){
    ...
    for(const candidate of candidates){
      const safe = this.safeImage(candidate);
      if(safe) return safe;
    }
    ...
    if(haystack.includes('hamb')) return 'assets/img/Hambg.jpeg';
    if(haystack.includes('alita')) return 'assets/img/Alitas.jpeg';
    if(haystack.includes('gordita')) return 'assets/img/Gorditas.jpeg';
    if(haystack.includes('quesa')) return 'assets/img/Quesa.jpeg';
    if(haystack.includes('hot') || haystack.includes('dog')) return 'assets/img/Hotdog.jpeg';
    if(haystack.includes('chicha')) return 'assets/img/Chicha.jpeg';
    if(haystack.includes('freski')) return 'assets/img/Freskis.jpeg';
    ...
    wrap.innerHTML = rows.map(item => {
      ...
      return `
        <article class="card menu-item">
          <div class="thumb">
            <img src="${this.escapeHtml(image)}" alt="${this.escapeHtml(item.nombre || 'Producto')}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='assets/img/Freskis.jpeg'">
          </div>
          <div>
