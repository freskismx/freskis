  ...
  contactListMarkup(item = {}, compact = false){
    ...
    if(website) rows.push(`
      <div class="contact-row">
        ...
        <a class="contact-value" href="${this.escapeHtml(website)}" target="_blank" rel="noopener noreferrer">${this.escapeHtml(website)}</a>
      </div>`);
    if(phone) rows.push(`
      <div class="contact-row">
        <span class="contact-label">Teléfono 1</span>
        <a class="contact-value" href="tel:${this.escapeHtml(phone.replace(/[^\d+]/g, ''))}">${this.escapeHtml(phone)}</a>
      </div>`);
    if(phone2) rows.push(`
      <div class="contact-row">
        <span class="contact-label">Teléfono 2</span>
        <a class="contact-value" href="tel:${this.escapeHtml(phone2.replace(/[^\d+]/g, ''))}">${this.escapeHtml(phone2)}</a>
      </div>`);
    if(email) rows.push(`
      <div class="contact-row">
        <span class="contact-label">Correo</span>
        <a class="contact-value" href="mailto:${this.escapeHtml(email)}">${this.escapeHtml(email)}</a>
      </div>`);
    if(!rows.length) return '';
    return `
      <div class="contact-card ${compact ? 'compact' : ''}">
