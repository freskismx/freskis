const APP = {
  ...
  contactListMarkup(item = {}, compact = false){
    ...
    const entries = [];
    if(website) entries.push(`<a class="contact-pill" href="${this.escapeHtml(website)}" target="_blank" rel="noopener noreferrer">🌐 Sitio web</a>`);
    if(phone) entries.push(`<a class="contact-pill" href="tel:${this.escapeHtml(phone.replace(/[^\d+]/g, ''))}">📞 ${this.escapeHtml(phone)}</a>`);
    if(phone2) entries.push(`<a class="contact-pill" href="tel:${this.escapeHtml(phone2.replace(/[^\d+]/g, ''))}">📱 ${this.escapeHtml(phone2)}</a>`);
    if(email) entries.push(`<a class="contact-pill" href="mailto:${this.escapeHtml(email)}">✉️ ${this.escapeHtml(email)}</a>`);
    if(!entries.length) return '';
    return `<div class="contact-pills ${compact ? 'compact' : ''}">${entries.join('')}</div>`;
  },
