const APP = {
  ...
  safeImage(src, fallback='assets/img/Freskis.jpeg'){
    ...
    const map = {
      ...
      'alitas.jpg': 'Alitas.jpeg',
      'hambg.jpeg': 'Hambg.jpeg',
      'hamburguesa.jpeg': 'Hambg.jpeg',
      'hamburguesas.jpeg': 'Hambg.jpeg',
      'hotdog.jpeg': 'Hotdog.jpeg',
      'hotdogs.jpeg': 'Hotdog.jpeg',
      'chicha.jpeg': 'Chicha.jpeg',
      'chicharron.jpeg': 'Chicha.jpeg',
      'freskis.jpeg': 'Freskis.jpeg',
      'gorditas.jpeg': 'Gorditas.jpeg',
      'quesa.jpeg': 'Quesa.jpeg',
      'quesadillas.jpeg': 'Quesa.jpeg'
    };
  ...
  resolveMenuImage(item){
    ...
    for(const candidate of candidates){
      const resolved = this.safeImage(candidate, '');
    ...
    if(/alita/.test(text)) return 'assets/img/Alitas.jpeg';
    if(/hot\s*dog|hotdog/.test(text)) return 'assets/img/Hotdog.jpeg';
    if(/gordita/.test(text)) return 'assets/img/Gorditas.jpeg';
    if(/quesa/.test(text)) return 'assets/img/Quesa.jpeg';
    if(/chicha|chicharr/.test(text)) return 'assets/img/Chicha.jpeg';
    if(/hamb|burger|star wars|titanic|gargamel|deadpool|hannibal|r[ií]o|pizza planet|hades|coco|mushu|nacho libre|rocky|thanos|hulk/.test(text)) return 'assets/img/Hambg.jpeg';
    if(/freski|fruta|topping|crema|top/.test(text)) return 'assets/img/Freskis.jpeg';

    return 'assets/img/Freskis.jpeg';
  },
  ...
  renderMenu(){
    ...
    wrap.innerHTML = rows.map(item => {
      ...
      const image = this.resolveMenuImage(item);
      return `
        <article class="card menu-item">
          <div class="thumb">
            <img src="${this.escapeHtml(image)}" alt="${this.escapeHtml(item.nombre || 'Producto')}" loading="lazy" onerror="this.onerror=null;this.src='assets/img/Freskis.jpeg'">
          </div>
          <div>
  ...
  renderAdminPanel(){
    ...
    this.el('promoFileInput')?.addEventListener('change', (e) => {
      ...
      this.el('promoStatus').textContent = `Archivo listo: ${file.name}`;
    });
    ...
    this.el('noticeFileInput')?.addEventListener('change', (e) => {
      ...
      this.el('noticeStatus').textContent = `Archivo listo: ${file.name}`;
    });
    ...
    this.el('musicFileInput')?.addEventListener('change', (e) => {
      ...
      this.el('musicStatus').textContent = `Archivo listo: ${file.name}`;
    });

    this.el('githubPushBtn').onclick = () => this.pushToGitHub();
  },
  ...
  async putGitHubFile(owner, repo, branch, path, content, token){
    ...
    const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, {
      ...
      body: JSON.stringify({
        message: `Actualiza ${path} desde panel admin Freskis`,
        content,
        branch,
        sha
      })
  ...
  async pushToGitHub(){
    ...
    const status = this.el('githubStatus');
    ...
    if(window.__pendingPromoFile){
      ...
      uploads.push({
        path:'data/promos.json',
        content: JSON.stringify([{ title:'Promoción actual', type: promoName.toLowerCase().endsWith('.pdf') ? 'pdf' : 'image', file:`assets/img/${promoName}`, active:true }], null, 2),
        type:'text'
      });
    }
    ...
    try{
      for(const item of uploads){
        const content = item.type === 'text'
          ? this.textToBase64(item.content)
          : await this.fileToBase64(item.file);
        await this.putGitHubFile(owner, repo, branch, item.path, content, token);
      }
