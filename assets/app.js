const APP = {
  ...
  async loadBaseData(){
    const [config, menu, freskiOptions, promos, games, sponsors, allies] = await Promise.all([
      ...
      this.fetchJson('data/sponsors.json').catch(() => ({})),
      this.fetchJson('data/allies.json').catch(() => ([]))
    ]);
    ...
    this.state.allies = Array.isArray(allies) ? allies : [];
  },
  ...
  bootIndex(){
    this.renderFilters();
    this.renderSponsor('sponsorHomeSlot', this.normalizeSponsor(this.state.sponsors?.mainSponsorHome, {title:'Patrocinador principal del mes'}), 'Patrocinador principal');
    this.renderMenu();
    this.renderCart();
  ...
  normalizeSponsor(entry, defaults={}){
    const source = entry || {};
    return {
      enabled: source.enabled !== false,
      name: this.safeText(source.name, defaults.name || ''),
      title: this.safeText(source.title, defaults.title || ''),
      description: this.safeText(source.description, defaults.description || ''),
      ctaText: this.safeText(source.ctaText, defaults.ctaText || 'Conoce más'),
      ctaUrl: this.safeText(source.ctaUrl, '#'),
      imageDesktop: this.safeImage(source.imageDesktop || defaults.imageDesktop || ''),
      imageMobile: this.safeImage(source.imageMobile || source.imageDesktop || defaults.imageMobile || defaults.imageDesktop || ''),
      logo: this.safeImage(source.logo || defaults.logo || ''),
      startDate: this.safeText(source.startDate, ''),
      endDate: this.safeText(source.endDate, '')
    };
  },

  activeWithinDates(item){
  ...
  renderSponsor(slotId, sponsor, label='Patrocinador'){
    const wrap = this.el(slotId);
    if(!wrap) return;
    if(!sponsor || sponsor.enabled === false || !this.activeWithinDates(sponsor)){
      wrap.innerHTML = '';
      return;
    }
    wrap.innerHTML = `
      <article class="card sponsor-card">
        <div class="sponsor-card__media">
          <picture>
            <source media="(max-width: 768px)" srcset="${this.escapeHtml(sponsor.imageMobile || sponsor.imageDesktop)}">
            <img src="${this.escapeHtml(sponsor.imageDesktop || sponsor.imageMobile)}" alt="${this.escapeHtml(sponsor.name || sponsor.title || label)}" onerror="this.style.display='none'">
          </picture>
        </div>
        <div class="sponsor-card__body">
          <span class="kicker">${this.escapeHtml(label)}</span>
          <h3>${this.escapeHtml(sponsor.title || sponsor.name || label)}</h3>
          ${sponsor.name ? `<p class="muted"><strong>${this.escapeHtml(sponsor.name)}</strong></p>` : ''}
          ${sponsor.description ? `<p>${this.escapeHtml(sponsor.description)}</p>` : ''}
          <div class="sponsor-card__actions">
            <a class="btn gold" href="${this.escapeHtml(sponsor.ctaUrl || '#')}" target="_blank" rel="noopener">${this.escapeHtml(sponsor.ctaText || 'Conoce más')}</a>
          </div>
        </div>
      </article>`;
  },

  renderAllies(slotId, filterKey='promos'){
    const wrap = this.el(slotId);
    if(!wrap) return;
    const allies = (this.state.allies || []).filter(item => item && item.enabled !== false && (!filterKey || (Array.isArray(item.showIn) && item.showIn.includes(filterKey))));
    if(!allies.length){
      wrap.innerHTML = '';
      return;
    }
    wrap.innerHTML = `
      <div class="section-title">
        <div>
          <h3>Aliados recomendados</h3>
          <p>Negocios locales y promociones sugeridas para nuestros clientes.</p>
        </div>
      </div>
      <div class="ally-grid">${allies.map(item => `
        <article class="card ally-card">
          <div class="ally-card__image">${item.image ? `<img src="${this.escapeHtml(this.safeImage(item.image))}" alt="${this.escapeHtml(item.name || 'Aliado')}" onerror="this.style.display='none'">` : ''}</div>
          <h4>${this.escapeHtml(item.name || 'Aliado')}</h4>
          ${item.category ? `<div class="muted">${this.escapeHtml(item.category)}</div>` : ''}
          ${item.description ? `<p>${this.escapeHtml(item.description)}</p>` : ''}
          <a class="btn secondary small" href="${this.escapeHtml(item.ctaUrl || '#')}" target="_blank" rel="noopener">${this.escapeHtml(item.ctaText || 'Ver más')}</a>
        </article>`).join('')}</div>`;
  },

  // ===================== promociones =====================
  bootPromos(){
    this.renderAllies('alliesPromoSlot', 'promos');
  ...
  // ===================== videojuegos =====================
  bootGames(){
    this.renderSponsor('sponsorGamesSlot', this.normalizeSponsor(this.state.sponsors?.mainSponsorGames, {title:'Patrocinador del entretenimiento'}), 'Patrocinador del entretenimiento');
  ...
  renderAdminPanel(){
    ...
    ['sponsorHomeDesktopInput','sponsorHomeMobileInput','sponsorHomeLogoInput','sponsorGamesDesktopInput','sponsorGamesMobileInput','sponsorGamesLogoInput'].forEach(id => {
      this.el(id)?.addEventListener('change', (e) => {
        const file = e.target.files?.[0];
        if(!file) return;
        window[`__pending_${id}`] = file;
        const statusId = id.startsWith('sponsorHome') ? 'sponsorHomeStatus' : 'sponsorGamesStatus';
        this.setAdminStatus(statusId, `Archivo listo: ${file.name}`);
        this.renderSyncQueueStatus();
      });
    });

    this.el('addAllyBtn')?.addEventListener('click', () => {
      this.state.allies = [...(this.state.allies || []), { enabled:true, name:'', category:'Aliado local', description:'', image:'', ctaText:'Ver promoción', ctaUrl:'', showIn:['promos'] }];
      this.renderAlliesEditor();
    });
    ...
    this.el('syncSponsorHomeBtn')?.addEventListener('click', () => this.pushSponsorsToGitHub('home'));
    this.el('syncSponsorGamesBtn')?.addEventListener('click', () => this.pushSponsorsToGitHub('games'));
    this.el('syncAlliesBtn')?.addEventListener('click', () => this.pushAlliesToGitHub());
    this.el('syncAllBtn')?.addEventListener('click', () => this.pushAllToGitHub());

    this.renderAdsAdminPanel();
  ...
  renderAdsAdminPanel(){
    ...
    const home = this.normalizeSponsor(sponsors.mainSponsorHome, {title:'Patrocinador principal del mes'});
    const games = this.normalizeSponsor(sponsors.mainSponsorGames, {title:'Patrocinador del entretenimiento'});

    const fill = (prefix, data) => {
      this.el(`${prefix}Enabled`).checked = !!data.enabled;
      this.el(`${prefix}Name`).value = data.name || '';
      this.el(`${prefix}Title`).value = data.title || '';
      this.el(`${prefix}Description`).value = data.description || '';
      this.el(`${prefix}CtaText`).value = data.ctaText || '';
      this.el(`${prefix}CtaUrl`).value = data.ctaUrl || '';
      this.el(`${prefix}StartDate`).value = data.startDate || '';
      this.el(`${prefix}EndDate`).value = data.endDate || '';
    };

    fill('sponsorHome', home);
    fill('sponsorGames', games);
    this.renderAlliesEditor();
  },

  renderAlliesEditor(){
    const wrap = this.el('alliesEditor');
    if(!wrap) return;
    const allies = Array.isArray(this.state.allies) ? this.state.allies : [];
    if(!allies.length){
      wrap.innerHTML = '<div class="empty">Aún no hay aliados capturados.</div>';
      return;
    }
    wrap.innerHTML = allies.map((ally, i) => `
      <article class="ally-editor-card">
        <div class="row">
          <div class="field"><label>Activo</label><input type="checkbox" data-ally-enabled="${i}" ${ally.enabled !== false ? 'checked' : ''}></div>
          <div class="field"><label>Nombre</label><input class="input" data-ally-name="${i}" value="${this.escapeHtml(ally.name || '')}"></div>
          <div class="field"><label>Categoría</label><input class="input" data-ally-category="${i}" value="${this.escapeHtml(ally.category || '')}"></div>
        </div>
        <div class="field"><label>Descripción</label><textarea data-ally-description="${i}">${this.escapeHtml(ally.description || '')}</textarea></div>
        <div class="row">
          <div class="field"><label>Texto botón</label><input class="input" data-ally-cta-text="${i}" value="${this.escapeHtml(ally.ctaText || '')}"></div>
          <div class="field"><label>URL</label><input class="input" data-ally-cta-url="${i}" value="${this.escapeHtml(ally.ctaUrl || '')}"></div>
          <div class="field"><label>Imagen/flyer</label><input class="input" type="file" data-ally-file="${i}" accept=".png,.jpg,.jpeg,.webp,.pdf"></div>
        </div>
        <div class="row ally-checks">
          <label><input type="checkbox" data-ally-show="${i}" value="promos" ${(ally.showIn || []).includes('promos') ? 'checked' : ''}> Promociones</label>
          <label><input type="checkbox" data-ally-show="${i}" value="postOrder" ${(ally.showIn || []).includes('postOrder') ? 'checked' : ''}> Después del pedido</label>
          <label><input type="checkbox" data-ally-show="${i}" value="games" ${(ally.showIn || []).includes('games') ? 'checked' : ''}> Videojuegos</label>
        </div>
        <div class="row">
          <button class="btn secondary small" type="button" data-remove-ally="${i}">Eliminar aliado</button>
        </div>
      </article>`).join('');

    wrap.querySelectorAll('[data-remove-ally]').forEach(btn => btn.addEventListener('click', () => {
      const idx = Number(btn.dataset.removeAlly);
      this.state.allies.splice(idx, 1);
      this.renderAlliesEditor();
    }));

    wrap.querySelectorAll('[data-ally-file]').forEach(inp => inp.addEventListener('change', (e) => {
      const idx = Number(inp.dataset.allyFile);
      const file = e.target.files?.[0];
      if(!file) return;
      window.__pendingAllyFiles = window.__pendingAllyFiles || {};
      window.__pendingAllyFiles[idx] = file;
      this.setAdminStatus('alliesStatus', `Aliado ${idx + 1}: archivo listo ${file.name}`);
      this.renderSyncQueueStatus();
    }));
  },

  collectSponsorsConfig(){
    return {
      mainSponsorHome: {
        enabled: !!this.el('sponsorHomeEnabled')?.checked,
        name: this.el('sponsorHomeName')?.value?.trim() || '',
        title: this.el('sponsorHomeTitle')?.value?.trim() || '',
        description: this.el('sponsorHomeDescription')?.value?.trim() || '',
        ctaText: this.el('sponsorHomeCtaText')?.value?.trim() || '',
        ctaUrl: this.el('sponsorHomeCtaUrl')?.value?.trim() || '',
        imageDesktop: this.state.sponsors?.mainSponsorHome?.imageDesktop || '',
        imageMobile: this.state.sponsors?.mainSponsorHome?.imageMobile || '',
        logo: this.state.sponsors?.mainSponsorHome?.logo || '',
        startDate: this.el('sponsorHomeStartDate')?.value || '',
        endDate: this.el('sponsorHomeEndDate')?.value || ''
      },
      mainSponsorGames: {
        enabled: !!this.el('sponsorGamesEnabled')?.checked,
        name: this.el('sponsorGamesName')?.value?.trim() || '',
        title: this.el('sponsorGamesTitle')?.value?.trim() || '',
        description: this.el('sponsorGamesDescription')?.value?.trim() || '',
        ctaText: this.el('sponsorGamesCtaText')?.value?.trim() || '',
        ctaUrl: this.el('sponsorGamesCtaUrl')?.value?.trim() || '',
        imageDesktop: this.state.sponsors?.mainSponsorGames?.imageDesktop || '',
        imageMobile: this.state.sponsors?.mainSponsorGames?.imageMobile || '',
        logo: this.state.sponsors?.mainSponsorGames?.logo || '',
        startDate: this.el('sponsorGamesStartDate')?.value || '',
        endDate: this.el('sponsorGamesEndDate')?.value || ''
      }
    };
  },

  collectAlliesConfig(){
    const cards = [...document.querySelectorAll('#alliesEditor .ally-editor-card')];
    return cards.map((card, i) => ({
      enabled: !!card.querySelector(`[data-ally-enabled="${i}"]`)?.checked,
      name: card.querySelector(`[data-ally-name="${i}"]`)?.value?.trim() || '',
      category: card.querySelector(`[data-ally-category="${i}"]`)?.value?.trim() || '',
      description: card.querySelector(`[data-ally-description="${i}"]`)?.value?.trim() || '',
      image: (this.state.allies?.[i]?.image) || '',
      ctaText: card.querySelector(`[data-ally-cta-text="${i}"]`)?.value?.trim() || '',
      ctaUrl: card.querySelector(`[data-ally-cta-url="${i}"]`)?.value?.trim() || '',
      showIn: [...card.querySelectorAll(`[data-ally-show="${i}"]:checked`)].map(x => x.value)
    }));
  },
  ...
  sponsorTargetPath(slot, variant, fileName){
    return `assets/ads/${slot}-${variant}-${fileName}`;
  },

  allyTargetPath(fileName){
    return `assets/ads/allies/${fileName}`;
  },
  ...
  async pushSponsorsToGitHub(which='all'){
    try{
      const sponsors = this.collectSponsorsConfig();
      const fileMap = [
        ['mainSponsorHome','desktop','sponsorHomeDesktopInput'],
        ['mainSponsorHome','mobile','sponsorHomeMobileInput'],
        ['mainSponsorHome','logo','sponsorHomeLogoInput'],
        ['mainSponsorGames','desktop','sponsorGamesDesktopInput'],
        ['mainSponsorGames','mobile','sponsorGamesMobileInput'],
        ['mainSponsorGames','logo','sponsorGamesLogoInput']
      ];
      this.setGitHubStatus('Publicando patrocinadores...');
      for(const [key, variant, inputId] of fileMap){
        const file = window[`__pending_${inputId}`];
        if(!file) continue;
        if(which === 'home' && key !== 'mainSponsorHome') continue;
        if(which === 'games' && key !== 'mainSponsorGames') continue;
        const safeName = this.slugFileName(file.name);
        const slot = key === 'mainSponsorHome' ? 'home-sponsor' : 'games-sponsor';
        const targetPath = this.sponsorTargetPath(slot, variant, safeName);
        await this.writeGitHubBinary(targetPath, file, `Actualizar ${slot} ${variant}`);
        if(variant === 'desktop') sponsors[key].imageDesktop = targetPath;
        if(variant === 'mobile') sponsors[key].imageMobile = targetPath;
        if(variant === 'logo') sponsors[key].logo = targetPath;
      }
      await this.writeGitHubText('data/sponsors.json', JSON.stringify(sponsors, null, 2), 'Actualizar patrocinadores');
      this.state.sponsors = sponsors;
      this.setGitHubStatus('Patrocinadores publicados correctamente.');
    }catch(err){
      console.error(err);
      this.setGitHubStatus(`Error al publicar patrocinadores: ${err.message}`, true);
    }
  },

  async pushAlliesToGitHub(){
    try{
      const allies = this.collectAlliesConfig();
      const pending = window.__pendingAllyFiles || {};
      this.setGitHubStatus('Publicando aliados...');
      for(const [idx, file] of Object.entries(pending)){
        const i = Number(idx);
        if(!allies[i] || !file) continue;
        const safeName = this.slugFileName(file.name);
        const targetPath = this.allyTargetPath(safeName);
        await this.writeGitHubBinary(targetPath, file, `Actualizar aliado ${safeName}`);
        allies[i].image = targetPath;
      }
      await this.writeGitHubText('data/allies.json', JSON.stringify(allies, null, 2), 'Actualizar aliados');
      this.state.allies = allies;
      this.setGitHubStatus('Aliados publicados correctamente.');
    }catch(err){
      console.error(err);
      this.setGitHubStatus(`Error al publicar aliados: ${err.message}`, true);
    }
  },
  ...
  async pushAllToGitHub(){
    try{
      ...
      await this.pushSponsorsToGitHub('all');
      await this.pushAlliesToGitHub();
