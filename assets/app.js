const APP = {
  ...
  renderAdminPanel(){
    ...
    this.el('addAllyBtn')?.addEventListener('click', () => {
      ...
      current.push({
        id: this.uid(),
        name: 'Nuevo aliado',
        description: 'Descripción breve del aliado.',
        image: 'assets/img/Logo-Freskis.jpeg',
        link: '#',
        button: 'Conocer',
        priority: current.length + 1
      });
      this.renderAlliesEditor(current);
      this.renderSyncQueueStatus();
    });
  ...
  renderAlliesEditor(allies = []){
      ...
      <article class="ally-editor-card" data-ally-id="${this.escapeHtml(ally.id)}">
        ...
        <div class="ally-editor-grid">
          <div>
            ...
            <div class="ally-editor-actions">
              <input class="input ally-image-file" type="file" accept=".png,.jpg,.jpeg,.webp,.gif">
            </div>
            <div class="muted ally-file-status"></div>
          </div>
          <div>
            <div class="row">
              <div class="field"><label>Nombre</label><input class="input ally-name" value="${this.escapeHtml(ally.name)}"></div>
              <div class="field"><label>Prioridad</label><input class="input ally-priority" type="number" value="${this.escapeHtml(ally.priority)}"></div>
            </div>
            <div class="field"><label>Descripción</label><textarea class="ally-description">${this.escapeHtml(ally.description)}</textarea></div>
            <div class="row">
              <div class="field"><label>Botón</label><input class="input ally-button" value="${this.escapeHtml(ally.button || 'Conocer')}"></div>
              <div class="field"><label>Enlace</label><input class="input ally-link" value="${this.escapeHtml(ally.link || '#')}"></div>
            </div>
            <div class="field"><label>Ruta de imagen</label><input class="input ally-image" value="${this.escapeHtml(ally.image || '')}"></div>
            <div class="ally-editor-actions">
    ...
    wrap.querySelectorAll('.ally-image').forEach(input => {
      input.addEventListener('input', (e) => {
        const card = e.target.closest('[data-ally-id]');
        const allyId = card?.dataset.allyId;
        this.updateImagePreview(`allyPreview-${allyId}`, e.target.value, 'assets/img/Logo-Freskis.jpeg');
      });
    });

    wrap.querySelectorAll('.ally-image-file').forEach(input => {
      input.addEventListener('change', (e) => {
        ...
        const pathInput = card.querySelector('.ally-image');
        if(pathInput) pathInput.value = targetPath;
        this.previewLocalFile(file, `allyPreview-${allyId}`, 'assets/img/Logo-Freskis.jpeg');
        const status = card.querySelector('.ally-file-status');
        if(status) status.textContent = `Imagen lista: ${file.name} → ${targetPath}`;
        this.renderSyncQueueStatus();
      });
    });
  ...
  collectAlliesFromEditor(){
    ...
    return cards.map((card, idx) => ({
      ...
      name: card.querySelector('.ally-name')?.value?.trim() || `Aliado ${idx + 1}`,
      description: card.querySelector('.ally-description')?.value?.trim() || '',
      image: card.querySelector('.ally-image')?.value?.trim() || 'assets/img/Logo-Freskis.jpeg',
      link: card.querySelector('.ally-link')?.value?.trim() || '#',
      button: card.querySelector('.ally-button')?.value?.trim() || 'Conocer',
      priority: Number(card.querySelector('.ally-priority')?.value || 0)
    })).filter(item => item.name);
  },
