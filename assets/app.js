const APP = {
  ...
  normalizeAdvertisingConfig(advertising = {}){
    ...
    return {
      top: {
        active: !!top.active,
        title: this.safeText(top.title, 'Patrocinador recomendado'),
        text: this.safeText(top.text, 'Este espacio puede mostrar una recomendación comercial sin afectar el flujo del pedido.'),
        button: this.safeText(top.button, 'Conocer más'),
        link: this.safeText(top.link, '#'),
        image: this.safeImage(top.image || 'assets/img/Flayer-Promocion.jpeg')
      },
      middle: {
        active: !!middle.active,
        title: this.safeText(middle.title, 'Negocio recomendado'),
        text: this.safeText(middle.text, 'Una segunda ubicación publicitaria, pensada para ser visible y elegante.'),
        button: this.safeText(middle.button, 'Ver más'),
        link: this.safeText(middle.link, '#'),
        image: this.safeImage(middle.image || 'assets/img/Flayer-Promocion.jpeg')
      },
  ...
  resetAllyForm(defaultPriority = null){
    ...
    const suggestedPriority = defaultPriority ?? Math.max(1, cards.length + 1);
  ...
  renderAlliesEditor(allies = []){
    ...
    wrap.innerHTML = normalized.map((ally, idx) => `
      <article class="ally-editor-card" data-ally-id="${this.escapeHtml(ally.id)}">
  ...
  collectAlliesFromEditor(){
    ...
    return cards.map((card, idx) => {
      const allyId = card.dataset.allyId || this.uid();
      const name = card.querySelector('.ally-editor-head strong')?.textContent?.trim() || `Aliado ${idx + 1}`;
      const description = card.querySelector('.ally-summary')?.textContent?.trim() || '';
      const image = card.querySelector('img')?.getAttribute('src')?.trim() || 'assets/img/Logo-Freskis.jpeg';
      const meta = Array.from(card.querySelectorAll('.ally-summary-grid span')).map(el => el.textContent || '');
      const button = (meta[0] || '').replace(/^\s*Botón:\s*/i, '').trim() || 'Conocer';
      const link = (meta[1] || '').replace(/^\s*Enlace:\s*/i, '').trim() || '#';
      const imageMeta = (meta[2] || '').replace(/^\s*Imagen:\s*/i, '').trim();
      const priorityText = card.querySelector('.priority-badge')?.textContent || '1';
      const priority = Number(String(priorityText).replace(/[^0-9.-]/g, '')) || 1;
      return {
        id: allyId,
        name,
        description,
