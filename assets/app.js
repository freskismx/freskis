const APP = {
  ...
  renderAdminPanel(){
    ...
    this.el('cfgAlliesJson').value = JSON.stringify(advertising.allies || [], null, 2);
  ...
  collectAdminConfig(){
    ...
    let allies = [];
    const alliesRaw = this.el('cfgAlliesJson')?.value?.trim() || '[]';
    try{
      allies = JSON.parse(alliesRaw);
      if(!Array.isArray(allies)) throw new Error('La lista de aliados debe ser un arreglo JSON.');
    }catch(err){
      throw new Error(`Aliados JSON inválido: ${err.message}`);
    }
