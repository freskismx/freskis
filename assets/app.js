const APP = {
  ...
  async loadBaseData(){
    ...
    this.state.config = this.normalizeWhatsappLabels(config || {});
  ...
  applyLocalPreviewOverrides(){
    ...
    try{
      ...
      this.state.config = this.normalizeWhatsappLabels({ ...this.state.config, ...parsed });
    }catch(err){
  ...
  renderCart(){
    ...
    if(waWrap){
      ...
      waWrap.innerHTML = phones.map((phone, i) =>
        `<button class="btn gold small" data-wa="${this.escapeHtml(phone)}">${this.escapeHtml(this.state.config?.whatsappLabel?.[i] || (i===0?`Solicita en Sucursal`:`Solicita a Domicilio`))}</button>`
      ).join('');
  ...
  renderAdminPanel(){
    ...
    this.el('jsonPreview').textContent = JSON.stringify(this.collectAdminConfig(), null, 2);
  ...
  collectAdminConfig(){
    ...
    cfg.whatsappLabel = ['Solicita en Sucursal', 'Solicita a Domicilio'];
