const APP = {
  ...
  safeImage(src){
    ...
    if(!s) return 'assets/img/Freskis.jpeg';
    if(/^https?:\/\//i.test(s) || s.startsWith('data:') || s.startsWith('/')) return s;
    const file = s.split('/').pop();
    const map = {
      'alitas.jpeg': 'Alitas.jpeg',
      'hambg.jpeg': 'Hambg.jpeg',
      'chicha.jpeg': 'Chicha.jpeg',
      'freskis.jpeg': 'Freskis.jpeg',
      'gorditas.jpeg': 'Gorditas.jpeg',
      'hotdog.jpeg': 'Hotdog.jpeg',
      'quesa.jpeg': 'Quesa.jpeg'
    };
    return `assets/img/${map[file.toLowerCase()] || file}`;
  },
  ...
  async loadBaseData(){
    ...
    this.state.config.advertising = this.normalizeAdvertisingConfig(this.state.config.advertising);
  ...
  renderShell(){
    ...
    this.renderAdvertisingSections();
  },

  normalizeAdvertisingConfig(advertising = {}){
    const top = advertising.top || {};
    const middle = advertising.middle || {};
    const allies = Array.isArray(advertising.allies) ? advertising.allies : [];
    const normalizeContact = (item = {}, fallbackTitle = 'Publicidad') => ({
      website: this.safeText(item.website || item.link, ''),
      phone: this.safeText(item.phone || item.phoneNumber || '', ''),
      phone2: this.safeText(item.phone2 || item.phoneSecondary || '', ''),
      email: this.safeText(item.email, ''),
      link: this.safeText(item.link || item.website || '#', '#'),
      title: this.safeText(item.title, fallbackTitle),
      text: this.safeText(item.text, ''),
      button: this.safeText(item.button, 'Conocer más'),
      image: this.safeImage(item.image || 'assets/img/Flayer-Promocion.jpeg')
    });
    return {
      top: {
        active: !!top.active,
        ...normalizeContact(top, 'Patrocinador recomendado'),
        text: this.safeText(top.text, 'Este espacio puede mostrar una recomendación comercial sin afectar el flujo del pedido.')
      },
      middle: {
        active: !!middle.active,
        ...normalizeContact(middle, 'Negocio recomendado'),
        text: this.safeText(middle.text, 'Una segunda ubicación publicitaria, pensada para ser visible y elegante.')
      },
      alliesEnabled: advertising.alliesEnabled !== false,
      allies: allies.map((item, idx) => ({
        id: this.safeText(item?.id, this.uid()),
        name: this.safeText(item?.name, `Aliado ${idx+1}`),
        description: this.safeText(item?.description, 'Negocio recomendado de la zona.'),
        image: this.safeImage(item?.image || 'assets/img/Logo-Freskis.jpeg'),
        link: this.safeText(item?.link || item?.website, '#'),
        website: this.safeText(item?.website || item?.link, ''),
        phone: this.safeText(item?.phone || item?.phoneNumber, ''),
        phone2: this.safeText(item?.phone2 || item?.phoneSecondary, ''),
        email: this.safeText(item?.email, ''),
        button: this.safeText(item?.button, 'Conocer'),
        priority: Number.isFinite(Number(item?.priority)) ? Number(item.priority) : (allies.length - idx)
      })).filter(item => item.name)
        .sort((a, b) => (Number(b.priority) || 0) - (Number(a.priority) || 0) || a.name.localeCompare(b.name, 'es'))
    };
  },

  contactListMarkup(item = {}, compact = false){
    const website = this.safeText(item.website || item.link, '');
    const phone = this.safeText(item.phone, '');
    const phone2 = this.safeText(item.phone2, '');
    const email = this.safeText(item.email, '');
    const rows = [];
    const pushCard = (icon, label, value, href = '') => {
      rows.push(`
        <a class="contact-item ${href ? '' : 'is-static'}" ${href ? `href="${href}" target="_blank" rel="noopener noreferrer"` : 'href="#" tabindex="-1" aria-disabled="true"'}>
          <span class="contact-icon" aria-hidden="true">${icon}</span>
          <span class="contact-copy">
            <span class="contact-label">${label}</span>
            <span class="contact-value">${this.escapeHtml(value)}</span>
          </span>
        </a>`);
    };
    if(website) pushCard('🌐', 'Sitio web', website, this.escapeHtml(website));
    if(phone) pushCard('📞', 'Teléfono 1', phone, `tel:${this.escapeHtml(phone.replace(/[^\d+]/g, ''))}`);
    if(phone2) pushCard('☎️', 'Teléfono 2', phone2, `tel:${this.escapeHtml(phone2.replace(/[^\d+]/g, ''))}`);
    if(email) pushCard('✉️', 'Correo electrónico', email, `mailto:${this.escapeHtml(email)}`);
    if(!rows.length) return '';
    return `
      <section class="contact-card ${compact ? 'compact' : ''}" aria-label="Datos de contacto del patrocinador o aliado">
        <div class="contact-card-head">
          <div>
            <div class="contact-card-kicker">Contacto directo</div>
            <div class="contact-card-title">Datos de contacto</div>
          </div>
        </div>
        <div class="contact-list ${compact ? 'compact' : ''}">${rows.join('')}</div>
      </section>`;
  },

  adCardMarkup(ad = {}, compact = false){
    const tag = compact ? 'Aliado comercial' : 'Patrocinado';
    const link = ad.link && ad.link !== '#' ? ad.link : '';
    const button = this.escapeHtml(ad.button || 'Conocer más');
    return `
      <article class="card sponsor-card ${compact ? 'compact' : ''}">
        <div class="sponsor-media">
          <img src="${this.escapeHtml(this.safeImage(ad.image))}" alt="${this.escapeHtml(ad.title || 'Publicidad')}" onerror="this.src='assets/img/Freskis.jpeg'">
        </div>
        <div class="sponsor-copy">
          <span class="sponsor-label">${tag}</span>
          <h4>${this.escapeHtml(ad.title || 'Publicidad')}</h4>
          <p>${this.escapeHtml(ad.text || '')}</p>
          ${this.contactListMarkup(ad, compact)}
          ${link ? `<a class="btn small secondary" href="${this.escapeHtml(link)}" target="_blank" rel="noopener noreferrer">${button}</a>` : ''}
        </div>
      </article>`;
  },

  renderAdvertisingSections(){
    const advertising = this.state.config?.advertising || this.normalizeAdvertisingConfig();
    const topWrap = this.el('adSlotTop');
    const topCard = this.el('adTopCard');
    if(topWrap && topCard){
      if(advertising.top?.active){
        topWrap.classList.remove('hidden');
        topCard.innerHTML = this.adCardMarkup(advertising.top);
      }else{
    ...
    const middleWrap = this.el('adSlotMiddle');
    const middleCard = this.el('adMiddleCard');
    if(middleWrap && middleCard){
      if(advertising.middle?.active){
        middleWrap.classList.remove('hidden');
        middleCard.innerHTML = this.adCardMarkup(advertising.middle);
      }else{
        middleWrap.classList.add('hidden');
        middleCard.innerHTML = '';
      }
    }

    const alliesSection = this.el('alliesSection');
    const alliesGrid = this.el('alliesGrid');
    if(alliesSection && alliesGrid){
      if(advertising.alliesEnabled && advertising.allies?.length){
        alliesSection.classList.remove('hidden');
        alliesGrid.innerHTML = advertising.allies.map(item => `
          <article class="card ally-card">
            <div class="ally-logo"><img src="${this.escapeHtml(this.safeImage(item.image))}" alt="${this.escapeHtml(item.name)}" onerror="this.src='assets/img/Logo-Freskis.jpeg'"></div>
            <h4>${this.escapeHtml(item.name)}</h4>
            <p>${this.escapeHtml(item.description || '')}</p>
            ${this.contactListMarkup(item, true)}
            ${item.link && item.link !== '#' ? `<a class="btn small secondary" href="${this.escapeHtml(item.link)}" target="_blank" rel="noopener noreferrer">${this.escapeHtml(item.button || 'Conocer')}</a>` : ''}
          </article>`).join('');
      }else{
        alliesSection.classList.add('hidden');
  ...
  renderAdminPanel(){
    ...
    const advertising = this.normalizeAdvertisingConfig(cfg.advertising);
    this.el('cfgAdTopActive').checked = !!advertising.top.active;
    this.el('cfgAdTopTitle').value = advertising.top.title || '';
    this.el('cfgAdTopText').value = advertising.top.text || '';
    this.el('cfgAdTopButton').value = advertising.top.button || '';
    this.el('cfgAdTopLink').value = advertising.top.link || '';
    this.el('cfgAdTopWebsite').value = advertising.top.website || '';
    this.el('cfgAdTopPhone').value = advertising.top.phone || '';
    this.el('cfgAdTopPhone2').value = advertising.top.phone2 || '';
    this.el('cfgAdTopEmail').value = advertising.top.email || '';
    this.el('cfgAdTopImage').value = advertising.top.image || '';
    this.el('cfgAdMiddleActive').checked = !!advertising.middle.active;
    this.el('cfgAdMiddleTitle').value = advertising.middle.title || '';
    this.el('cfgAdMiddleText').value = advertising.middle.text || '';
    this.el('cfgAdMiddleButton').value = advertising.middle.button || '';
    this.el('cfgAdMiddleLink').value = advertising.middle.link || '';
    this.el('cfgAdMiddleWebsite').value = advertising.middle.website || '';
    this.el('cfgAdMiddlePhone').value = advertising.middle.phone || '';
    this.el('cfgAdMiddlePhone2').value = advertising.middle.phone2 || '';
    this.el('cfgAdMiddleEmail').value = advertising.middle.email || '';
    this.el('cfgAdMiddleImage').value = advertising.middle.image || '';
    this.el('cfgAlliesEnabled').checked = !!advertising.alliesEnabled;
    this.updateImagePreview('cfgAdTopPreview', advertising.top.image, 'assets/img/Flayer-Promocion.jpeg');
    this.updateImagePreview('cfgAdMiddlePreview', advertising.middle.image, 'assets/img/Freskis.jpeg');
    this.renderAlliesEditor(advertising.allies || []);
    this.resetAllyForm();
    ...
    this.el('cfgAdTopImage')?.addEventListener('input', (e) => {
      this.updateImagePreview('cfgAdTopPreview', e.target.value, 'assets/img/Flayer-Promocion.jpeg');
    });
    this.el('cfgAdMiddleImage')?.addEventListener('input', (e) => {
      this.updateImagePreview('cfgAdMiddlePreview', e.target.value, 'assets/img/Freskis.jpeg');
    });

    this.el('cfgAdTopImageFile')?.addEventListener('change', (e) => {
      const file = e.target.files?.[0];
      if(!file) return;
      const safeName = this.slugFileName(file.name);
      const targetPath = this.imageTargetPath(safeName);
      window.__pendingAdTopFile = file;
      this.el('cfgAdTopImage').value = targetPath;
      this.previewLocalFile(file, 'cfgAdTopPreview', 'assets/img/Flayer-Promocion.jpeg');
      this.setAdminStatus('cfgAdTopUploadStatus', `Imagen lista: ${file.name} → ${targetPath}`);
      this.renderSyncQueueStatus();
    });

    this.el('cfgAdMiddleImageFile')?.addEventListener('change', (e) => {
      const file = e.target.files?.[0];
      if(!file) return;
      const safeName = this.slugFileName(file.name);
      const targetPath = this.imageTargetPath(safeName);
      window.__pendingAdMiddleFile = file;
      this.el('cfgAdMiddleImage').value = targetPath;
      this.previewLocalFile(file, 'cfgAdMiddlePreview', 'assets/img/Freskis.jpeg');
      this.setAdminStatus('cfgAdMiddleUploadStatus', `Imagen lista: ${file.name} → ${targetPath}`);
      this.renderSyncQueueStatus();
    });

    this.el('addAllyBtn')?.addEventListener('click', () => {
      this.resetAllyForm();
      this.el('allyFormName')?.focus();
    });

    this.el('saveAllyBtn')?.addEventListener('click', () => {
      this.saveAllyFromForm();
    });

    this.el('resetAllyFormBtn')?.addEventListener('click', () => {
      this.resetAllyForm();
    });

    this.el('allyFormImage')?.addEventListener('input', (e) => {
      this.updateImagePreview('allyFormPreview', e.target.value, 'assets/img/Logo-Freskis.jpeg');
    });

    this.el('allyFormImageFile')?.addEventListener('change', (e) => {
      const file = e.target.files?.[0];
      if(!file) return;
      const safeName = this.slugFileName(file.name);
      const targetPath = this.imageTargetPath(safeName);
      const allyId = this.el('allyFormId')?.value || this.uid();
      this.el('allyFormId').value = allyId;
      window.__pendingAllyFiles = window.__pendingAllyFiles || {};
      window.__pendingAllyFiles[allyId] = file;
      this.el('allyFormImage').value = targetPath;
      this.previewLocalFile(file, 'allyFormPreview', 'assets/img/Logo-Freskis.jpeg');
      this.setAdminStatus('allyFormUploadStatus', `Imagen lista: ${file.name} → ${targetPath}`);
      this.renderSyncQueueStatus();
    });
    ...
    this.el('imageFileInput')?.addEventListener('change', (e) => {
      const file = e.target.files?.[0];
      if(!file) return;
      window.__pendingImageFile = file;
      this.setAdminStatus('imageStatus', `Imagen lista: ${file.name}`);
      this.renderSyncQueueStatus();
    });
    ...
    this.el('syncImageBtn')?.addEventListener('click', () => this.pushImageToGitHub());
  ...
  collectAdminConfig(){
    ...
    const allies = this.collectAlliesFromEditor();

    cfg.advertising = this.normalizeAdvertisingConfig({
      top: {
        active: this.el('cfgAdTopActive').checked,
        title: this.el('cfgAdTopTitle').value.trim(),
        text: this.el('cfgAdTopText').value.trim(),
        button: this.el('cfgAdTopButton').value.trim(),
        link: this.el('cfgAdTopLink').value.trim(),
        website: this.el('cfgAdTopWebsite').value.trim(),
        phone: this.el('cfgAdTopPhone').value.trim(),
        phone2: this.el('cfgAdTopPhone2').value.trim(),
        email: this.el('cfgAdTopEmail').value.trim(),
        image: this.el('cfgAdTopImage').value.trim()
      },
      middle: {
        active: this.el('cfgAdMiddleActive').checked,
        title: this.el('cfgAdMiddleTitle').value.trim(),
        text: this.el('cfgAdMiddleText').value.trim(),
        button: this.el('cfgAdMiddleButton').value.trim(),
        link: this.el('cfgAdMiddleLink').value.trim(),
        website: this.el('cfgAdMiddleWebsite').value.trim(),
        phone: this.el('cfgAdMiddlePhone').value.trim(),
        phone2: this.el('cfgAdMiddlePhone2').value.trim(),
        email: this.el('cfgAdMiddleEmail').value.trim(),
        image: this.el('cfgAdMiddleImage').value.trim()
      },
      alliesEnabled: this.el('cfgAlliesEnabled').checked,
      allies
    });
  ...
  updateImagePreview(targetId, src, fallback = 'assets/img/Freskis.jpeg'){
    const img = this.el(targetId);
    if(!img) return;
    img.src = this.safeImage(src || fallback);
    img.onerror = () => { img.src = fallback; };
  },

  previewLocalFile(file, targetId, fallback = 'assets/img/Freskis.jpeg'){
    const img = this.el(targetId);
    if(!img || !file) return;
  ...
  resetAllyForm(defaultPriority = null){
    const current = this.collectAlliesFromEditor();
    const maxPriority = current.reduce((max, item) => Math.max(max, Number(item.priority) || 0), 0);
    const suggestedPriority = defaultPriority ?? Math.max(1, maxPriority + 1);
    if(this.el('allyFormId')) this.el('allyFormId').value = '';
    if(this.el('allyFormTitle')) this.el('allyFormTitle').textContent = 'Captura de aliado';
    if(this.el('allyFormMode')) this.el('allyFormMode').textContent = 'Nuevo';
    if(this.el('allyFormName')) this.el('allyFormName').value = '';
    if(this.el('allyFormPriority')) this.el('allyFormPriority').value = String(suggestedPriority);
    if(this.el('allyFormDescription')) this.el('allyFormDescription').value = '';
    if(this.el('allyFormButton')) this.el('allyFormButton').value = 'Conocer';
    if(this.el('allyFormLink')) this.el('allyFormLink').value = '#';
    if(this.el('allyFormWebsite')) this.el('allyFormWebsite').value = '';
    if(this.el('allyFormPhone')) this.el('allyFormPhone').value = '';
    if(this.el('allyFormPhone2')) this.el('allyFormPhone2').value = '';
    if(this.el('allyFormEmail')) this.el('allyFormEmail').value = '';
    if(this.el('allyFormImage')) this.el('allyFormImage').value = 'assets/img/Logo-Freskis.jpeg';
    if(this.el('allyFormImageFile')) this.el('allyFormImageFile').value = '';
    this.updateImagePreview('allyFormPreview', 'assets/img/Logo-Freskis.jpeg', 'assets/img/Logo-Freskis.jpeg');
    this.setAdminStatus('allyFormUploadStatus', '');
  },

  fillAllyForm(ally = {}){
    const allyId = ally.id || this.uid();
    if(this.el('allyFormId')) this.el('allyFormId').value = allyId;
    if(this.el('allyFormTitle')) this.el('allyFormTitle').textContent = `Editar aliado: ${ally.name || 'sin nombre'}`;
    if(this.el('allyFormMode')) this.el('allyFormMode').textContent = 'Edición';
    if(this.el('allyFormName')) this.el('allyFormName').value = ally.name || '';
    if(this.el('allyFormPriority')) this.el('allyFormPriority').value = String(Number(ally.priority) || 1);
    if(this.el('allyFormDescription')) this.el('allyFormDescription').value = ally.description || '';
    if(this.el('allyFormButton')) this.el('allyFormButton').value = ally.button || 'Conocer';
    if(this.el('allyFormLink')) this.el('allyFormLink').value = ally.link || '#';
    if(this.el('allyFormWebsite')) this.el('allyFormWebsite').value = ally.website || '';
    if(this.el('allyFormPhone')) this.el('allyFormPhone').value = ally.phone || '';
    if(this.el('allyFormPhone2')) this.el('allyFormPhone2').value = ally.phone2 || '';
    if(this.el('allyFormEmail')) this.el('allyFormEmail').value = ally.email || '';
    if(this.el('allyFormImage')) this.el('allyFormImage').value = ally.image || 'assets/img/Logo-Freskis.jpeg';
    if(this.el('allyFormImageFile')) this.el('allyFormImageFile').value = '';
    this.updateImagePreview('allyFormPreview', ally.image || 'assets/img/Logo-Freskis.jpeg', 'assets/img/Logo-Freskis.jpeg');
    this.setAdminStatus('allyFormUploadStatus', '');
  },

  saveAllyFromForm(){
    const allyId = this.el('allyFormId')?.value?.trim() || this.uid();
    const ally = {
      id: allyId,
      name: this.el('allyFormName')?.value?.trim() || '',
      description: this.el('allyFormDescription')?.value?.trim() || '',
      image: this.el('allyFormImage')?.value?.trim() || 'assets/img/Logo-Freskis.jpeg',
      link: this.el('allyFormLink')?.value?.trim() || '#',
      website: this.el('allyFormWebsite')?.value?.trim() || '',
      phone: this.el('allyFormPhone')?.value?.trim() || '',
      phone2: this.el('allyFormPhone2')?.value?.trim() || '',
      email: this.el('allyFormEmail')?.value?.trim() || '',
      button: this.el('allyFormButton')?.value?.trim() || 'Conocer',
      priority: Number(this.el('allyFormPriority')?.value || 0) || 1
    };
    if(!ally.name){
      alert('Escribe al menos el nombre del aliado.');
      this.el('allyFormName')?.focus();
      return;
    }
    const allies = this.collectAlliesFromEditor();
    const index = allies.findIndex(item => item.id === allyId);
    if(index >= 0){
      allies[index] = { ...allies[index], ...ally };
    }else{
      allies.push(ally);
    }
    this.renderAlliesEditor(allies);
    this.resetAllyForm();
    this.renderSyncQueueStatus();
  },

  renderAlliesEditor(allies = []){
    const wrap = this.el('alliesEditorList');
    if(!wrap) return;
    const normalized = this.normalizeAdvertisingConfig({ allies }).allies;
    wrap.innerHTML = normalized.map((ally, idx) => `
      <article class="ally-editor-card" data-ally-id="${this.escapeHtml(ally.id)}" data-ally-name="${this.escapeHtml(ally.name)}" data-ally-description="${this.escapeHtml(ally.description)}" data-ally-image="${this.escapeHtml(ally.image)}" data-ally-link="${this.escapeHtml(ally.link)}" data-ally-website="${this.escapeHtml(ally.website || '')}" data-ally-phone="${this.escapeHtml(ally.phone || '')}" data-ally-phone2="${this.escapeHtml(ally.phone2 || '')}" data-ally-email="${this.escapeHtml(ally.email || '')}" data-ally-button="${this.escapeHtml(ally.button)}" data-ally-priority="${this.escapeHtml(ally.priority)}">
        <div class="ally-editor-head">
          <strong>${this.escapeHtml(ally.name || `Aliado ${idx + 1}`)}</strong>
          <div class="priority-badge">P ${this.escapeHtml(ally.priority)}</div>
        </div>
        <div class="ally-editor-grid compact">
          <div>
            <div class="ally-editor-preview"><img id="allyPreview-${this.escapeHtml(ally.id)}" src="${this.escapeHtml(this.safeImage(ally.image))}" alt="${this.escapeHtml(ally.name)}" onerror="this.src='assets/img/Logo-Freskis.jpeg'"></div>
          </div>
          <div>
            <p class="muted ally-summary">${this.escapeHtml(ally.description || 'Sin descripción todavía.')}</p>
            <div class="ally-summary-grid">
              <span><strong>Botón:</strong> ${this.escapeHtml(ally.button || 'Conocer')}</span>
              <span><strong>Enlace:</strong> ${this.escapeHtml(ally.link || '#')}</span>
              <span><strong>Sitio:</strong> ${this.escapeHtml(ally.website || 'No capturado')}</span>
              <span><strong>Tel. 1:</strong> ${this.escapeHtml(ally.phone || 'No capturado')}</span>
              <span><strong>Tel. 2:</strong> ${this.escapeHtml(ally.phone2 || 'No capturado')}</span>
              <span><strong>Correo:</strong> ${this.escapeHtml(ally.email || 'No capturado')}</span>
              <span><strong>Imagen:</strong> ${this.escapeHtml(ally.image || 'assets/img/Logo-Freskis.jpeg')}</span>
            </div>
            <div class="ally-editor-actions wrap">
              <button class="btn ally-edit" type="button">Editar</button>
              <button class="btn secondary ally-remove" type="button">Eliminar</button>
            </div>
          </div>
        </div>
      </article>
    ...
    wrap.querySelectorAll('.ally-edit').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const card = e.target.closest('[data-ally-id]');
        const allyId = card?.dataset.allyId;
        const ally = normalized.find(item => item.id === allyId);
        if(!ally) return;
        this.fillAllyForm(ally);
        this.el('allyFormName')?.focus();
        document.querySelector('.ally-capture-panel')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      });
    });

    wrap.querySelectorAll('.ally-remove').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const card = e.target.closest('[data-ally-id]');
        const allyId = card?.dataset.allyId;
        if(allyId && window.__pendingAllyFiles){
          delete window.__pendingAllyFiles[allyId];
        }
        if(this.el('allyFormId')?.value === allyId){
          this.resetAllyForm();
        }
        card?.remove();
        this.renderSyncQueueStatus();
  ...
  collectAlliesFromEditor(){
    const cards = Array.from(document.querySelectorAll('#alliesEditorList [data-ally-id]'));
    return cards.map((card, idx) => ({
      id: card.dataset.allyId || this.uid(),
      name: this.safeText(card.dataset.allyName, `Aliado ${idx + 1}`),
      description: this.safeText(card.dataset.allyDescription, ''),
      image: this.safeText(card.dataset.allyImage, 'assets/img/Logo-Freskis.jpeg'),
      link: this.safeText(card.dataset.allyLink, '#'),
      website: this.safeText(card.dataset.allyWebsite, ''),
      phone: this.safeText(card.dataset.allyPhone, ''),
      phone2: this.safeText(card.dataset.allyPhone2, ''),
      email: this.safeText(card.dataset.allyEmail, ''),
      button: this.safeText(card.dataset.allyButton, 'Conocer'),
      priority: Number(card.dataset.allyPriority || 0) || (idx + 1)
    })).filter(item => item.name);
  },

  async pushAdvertisingAssets(){
    if(window.__pendingAdTopFile){
      const safeName = this.slugFileName(window.__pendingAdTopFile.name);
      await this.writeGitHubBinary(this.imageTargetPath(safeName), window.__pendingAdTopFile, `Actualizar anuncio superior ${safeName}`);
    }
    if(window.__pendingAdMiddleFile){
      const safeName = this.slugFileName(window.__pendingAdMiddleFile.name);
      await this.writeGitHubBinary(this.imageTargetPath(safeName), window.__pendingAdMiddleFile, `Actualizar anuncio intermedio ${safeName}`);
    }
    const allyFiles = window.__pendingAllyFiles || {};
    for(const [allyId, file] of Object.entries(allyFiles)){
      if(!file) continue;
      const safeName = this.slugFileName(file.name);
      await this.writeGitHubBinary(this.imageTargetPath(safeName), file, `Actualizar logo aliado ${allyId}`);
    }
  },
  ...
  renderSyncQueueStatus(){
    ...
    if(window.__pendingAdTopFile) lines.push('• Imagen del anuncio superior lista');
    if(window.__pendingAdMiddleFile) lines.push('• Imagen del anuncio intermedio lista');
    const pendingAllies = Object.keys(window.__pendingAllyFiles || {}).length;
    if(pendingAllies) lines.push(`• ${pendingAllies} imagen(es) de aliados lista(s)`);
    if(window.__pendingMusicFile) lines.push('• Música lista');
    if(window.__pendingImageFile) lines.push('• Imagen de producto lista');
  ...
  async pushConfigToGitHub(){
    try{
      ...
      await this.pushAdvertisingAssets();
      await this.writeGitHubText('data/site-config.json', JSON.stringify(nextCfg, null, 2), 'Actualizar configuración general');
      localStorage.setItem('admin-preview-config', JSON.stringify(nextCfg));
      this.state.config = nextCfg;
      window.__pendingAdTopFile = null;
      window.__pendingAdMiddleFile = null;
      window.__pendingAllyFiles = {};
  ...
  async pushImageToGitHub(){
    try{
      if(!window.__pendingImageFile){
        throw new Error('Primero selecciona una imagen de producto.');
      }
      const safeName = this.slugFileName(window.__pendingImageFile.name);
      const targetPath = this.imageTargetPath(safeName);
      this.setGitHubStatus('Publicando imagen de producto...');
      await this.writeGitHubBinary(targetPath, window.__pendingImageFile, `Actualizar imagen ${safeName}`);
      this.setGitHubStatus(`Imagen publicada correctamente en ${targetPath}.`);
    }catch(err){
      console.error(err);
      this.setGitHubStatus(`Error al publicar imagen: ${err.message}`, true);
    }
  },
  ...
  async pushAllToGitHub(){
    try{
      ...
      if(window.__pendingImageFile) await this.pushImageToGitHub();
